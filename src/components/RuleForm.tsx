...
const mockRule = {
  ...
  criteria: [
    { id: 'c1', type: 'payee', operator: 'contains', value: 'Kroger' },
    { id: 'c2', type: 'payee', operator: 'contains', value: 'Walmart' },
    { id: 'c3', type: 'payee', operator: 'contains', value: 'Trader Joe\'s' },
  ],
  actions: [
    { id: 'a1', type: 'category', value: 'Groceries' },
  ],
};

const criteriaOptions = [
  { value: 'payee', label: 'Payee' },
  { value: 'memo', label: 'Memo Content' },
  { value: 'amount', label: 'Transaction Amount' },
  { value: 'date', label: 'Transaction Date' },
  { value: 'account', label: 'Account Name/Type' },
];
...
const actionOptions = [
  { value: 'category', label: 'Assign Category' },
  { value: 'flag', label: 'Add Flag' },
  { value: 'memo', label: 'Modify Memo' },
...
const RuleForm: React.FC = () => {
  ...
  const [criteria, setCriteria] = useState(isEditMode ? mockRule.criteria : [
    { id: 'new-c1', type: 'payee', operator: 'contains', value: '' }
  ]);
  const [actions, setActions] = useState(isEditMode ? mockRule.actions : [
    { id: 'new-a1', type: 'category', value: '' }
  ]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log({ name, description, criteria, actions, relationOperator });
    ...
    <form onSubmit={handleSubmit} className="space-y-8 animate-slide-in">
        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
            {criteria.map((criterion, index) => (
              <div 
                key={criterion.id} 
                className="flex items-start space-x-2 p-3 rounded-md bg-white/40 border border-gray-100 shadow-sm"
              >
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          ...
                          >
                            ...
                            {criterion.value ? (
                              format(new Date(criterion.value), "PPP")
                            ) : (
            ...
            <Button
              ...
              className="mt-2 glass-card hover:bg-app-blue hover:text-white"
            >
                  ...
                  <div className="sm:col-span-7">
                    {action.type === 'category' ? (
                      ...
                      <Input
                        ...
                        placeholder={`Enter ${action.type === 'memo' ? 'memo text' : 'split details'}`}
                        className="glass-card"
                      />
                    )}
                  </div>
      ...
      <div className="flex justify-between items-center pt-2">
        {isEditMode && (
          <Button 
            type="button" 
            className="bg-app-delete hover:bg-app-delete/90 text-white"
          >
