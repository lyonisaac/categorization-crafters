          ...
          <TableBody>
            {filteredRules.map((rule, index) => <TableRow key={rule.id} className={`transition-all-300 hover:bg-white/70 ${index % 2 === 0 ? 'bg-white/30' : 'bg-white/10'}`}>
                ...
                <TableCell className="text-right space-x-2">
                  <Button variant="outline" size="icon" onClick={() => navigate(`/rule-editor/${rule.id}`)} title="Edit Rule" className="h-8 w-8 transition-all-300 hover:text-app-blue hover:border-app-blue hover:bg-app-blue/10 hover:shadow-sm">
